...
public static class RangeScale {
    ...
    public static float MapPercentage(float value, float outMin, float outMid, float outMax) => Map(value, 0, 0.5f, 1, outMin, outMid, outMax);

    public static float Map(float value, float inMin, float inMax, float outMin, float outMax) {
        (inMin, inMax) = inMin > inMax ? (inMax, inMin) : (inMin, inMax);
        (outMin, outMax) = outMin > outMax ? (outMax, outMin) : (outMin, outMax);
        return Map(value, inMin, ((inMax - inMin) / 2) + inMin, inMax, outMin, ((outMax - outMin) / 2) + outMin, outMax);
    }
